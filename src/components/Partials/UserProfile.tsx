...
const UserProfile = ({user,isLoading}:{user:any,isLoading:boolean}) => {
    return (
        <>
            <div
                ...
                <div className='w-fit flex justify-center items-center gap-x-2'>
                    ...
                    <div>
                        ...
                        <p className='text-sm text-gray-400'>{user?.tweets?.length} Post</p>
                    </div>
                </div>
            </div>
            <div className='w-full'>
                ...
                <div className='w-full px-2 md:px-5 mt-[-75px] mb-3 flex justify-between items-end'>
                    <div className='w-[150px] h-[150px] border-2 border-black rounded-full bg-gray-500 relative overflow-hidden'>
                        <Image src={!user?.profile_avatar ? "/avatar.jpg" : user?.profile_avatar}
                               alt={"Profile"}
                ...
                <div className="w-full px-2 md:px-5">
                    <div className='w-full flex flex-col'>
                        <h1 className='text-white font-semibold text-xl'>{isLoading ? <LoadingCircle/> : user?.name}</h1>
                    ...
                    <div className="w-full flex gap-3 flex-wrap my-3">
                        {
                            user?.location && <div className='w-fit flex text-gray-400 text-sm justify-center items-center gap-x-2'>
                                <CiLocationOn/>
                                <div>
                                    {user?.location}
                                </div>
                        ...
                        {user?.website && <div
                            ...
                            <Link href={user?.website} className="text-blue-500 hover:underline " target={"_blank"}>
                                {user?.website}
                            </Link>
                        </div>}
                    ...
                    <div className="w-full flex gap-3 flex-wrap my-3">
                        <Link href={"/profile/following"} className='w-fit hover:underline flex text-gray-400 text-sm justify-center items-center gap-x-2'>
                            {!isLoading && user?.followings.length + " following"}
                        </Link>
                        <Link href={"/profile/followers"} className='w-fit hover:underline flex text-gray-400 text-sm justify-center items-center gap-x-2'>
                            {!isLoading && user?.followers.length + " follower"}
                        </Link>
                    </div>
